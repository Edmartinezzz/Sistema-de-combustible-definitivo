...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-white flex text-slate-900 overflow-x-hidden">
              ...
              >
                ...
                {isSidebarOpen && (
                  <motion.span 
                    initial={{ opacity: 0 }} 
                    animate={{ opacity: 1 }} 
                    className="ml-4 font-black text-xs uppercase italic tracking-tight"
                  >
                    {item.label}
                  </motion.span>
                )}
      ...
      <main className={`flex-1 w-full min-h-screen transition-all duration-300 ${isSidebarOpen ? 'md:ml-[280px]' : 'md:ml-[80px]'}`}>
